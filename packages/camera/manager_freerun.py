...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.test_file_saving:
            ...
            Parallel(n_jobs=8, prefer="threads")(delayed(test_saving_file)(k) for k in _store_buffer)
            _t_end_test_file_saving = datetime.now()
            print(f"Elapsed Time (storing into redis) : {(_t_start_test_file_saving-_t_end_test_file_saving).total_seconds()}")

        elif args.test_redis_saving:
            ...
            try:
                ...
                for cam_idx, camera_buffer in enumerate(_store_buffer):
                    for idx, image in camera_buffer:
            ...
            print(f"Elapsed Time (storing into redis) : {(_t_start_test_redis_saving-_t_end_test_redis_saving).total_seconds()}")
